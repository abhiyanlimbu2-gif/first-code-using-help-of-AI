...
// Get user's email
$stmt = $conn->prepare("SELECT email, full_name FROM users WHERE user_id = ? LIMIT 1");
...
// Fetch bookings that match the user's email
$stmt = $conn->prepare("SELECT b.*, bk.bike_name, bk.bike_model FROM bookings b LEFT JOIN bikes bk ON b.bike_id=bk.bike_id WHERE b.customer_email = ? ORDER BY b.start_date DESC");
